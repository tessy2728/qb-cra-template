...
const paramsToObjects = queryParams => {
...
const sortByAplphabetical = (data, key) => {
...
const formatFilterItems = (list, labelField, valueField, subType) => {
  if (labelField && valueField) return list?.map?.(ele => ({
    label: ele[labelField],
    value: ele[valueField],
    ...(subType ? {
      subType
    } : {})
  })) || [];else return list?.map?.(ele => ({
    label: ele,
    value: ele
  })) || [];
};
...
export function isEmptyValue(element) {
  let flag = false;
  flag = isEmpty(element);
  if (!flag) {
    flag = element == 'null' || element == 'undefined' || element == 0 || element == '0';
  }
  return flag;
}
export function isEmpty(element) {
  return !element || element == 'null' || element.length == 0 || element.size == 0;
}
export function isGreaterThanZero(element) {
...
export const maskPhone = phone => {
  if (phone) {
    ...
    return phoneNumberSplits ? '(' + phoneNumberSplits[1] + ') ' + phoneNumberSplits[2] + '-' + phoneNumberSplits[3] : phone;
  } else return phone;
};
export const maskEmail = email => {
  return email.replace(/^(.)(.*)(..@.*)$/, (_, a, b, c) => a + b.replace(/./g, 'X') + c);
};
...
export { pascalCase, paramsToObjects, formatFilterItems, removeWhiteSpaces, sortByAplphabetical };